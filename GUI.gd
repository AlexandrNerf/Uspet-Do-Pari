...
func gui_draw(player, andrei):
	...
	if seconds < 10:
		s0 = '0'
	hp.text = m0 + str(minutes) + ":" + s0 + str(seconds)
	#hp_bar.set_size(Vector2((int(player.HP*200/player.MAX_HP)), 35))
	mp_bar.set_size(Vector2((int(andrei.MP*105/andrei.MAX_MP)), 35))
	stamina_bar.set_size(Vector2(int((player.STAMINA*80)/player.MAX_STAMINA), 38))
