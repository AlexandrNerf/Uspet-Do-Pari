...
# Спрайты задаются через id атласа и координаты
...
func dig_room(xt, yt):
	...
	for x in range(new_room.positions.position.x, new_room.positions.position.x + new_room.positions.size.x +1):
		...
		#var mover = preload("res://Forest/Etc/mover.tscn").instantiate()
		#add_child(mover)
		#mover.position = Vector2(new_room.positions.position.x, new_room.positions.position.y)
...
func _ready():
	...
	#$DanilNarvakin/Camera2D.enabled = false
	#get_tree().change_scene_to_file("res://Forest/animation.tscn")
	#level_num = 3
...
func nsu_ready():
	...
	for room in rooms:
		doors_gnu(0, room)
		room.closed = true
	player.position = Vector2i(-15166.96, 8247.935)
...
func forest_ready():
	...
	for room in rooms:
		...
		#for x in range(room.positions.position.x+3, room.positions.position.x+10):
			#for y in range(room.positions.position.y+3, room.positions.position.y+8):
				#if x == room.positions.position.x+3 or x == room.positions.position.x+9 or y == room.positions.position.y+3 or y == room.positions.position.y+7:
					#var icy = Vector2i(x, y)
					#var sx = (icy.x*32) - icy.y*32 + 32
					#var sy = icy.x*16+icy.y*16+16
					#var ice = ice_floor.instantiate()
					#ice.position = Vector2i(sx, sy)
					#add_child(ice)
					
		var monst_in_room = abs(randi() % 2) + 1
		
		for j in range(monst_in_room):
			#var pos_x = room.positions.position.x + abs(randi() % int(room.positions.size.x)) + 1
			#var pos_y = room.positions.position.y + abs(randi() % int(room.positions.size.y)) + 1
			var monstr_coords = Vector2(room.positions.position.x + 1 + randi() % int(room.positions.size.x - 1), room.positions.position.y + 1 + randi() % int(room.positions.size.y - 1))
			var dogs = doge.instantiate()
			dogs.position = isom(monstr_coords.x, monstr_coords.y)
			room.monsters.append(dogs)
			add_child(dogs)
	...
	#$CanvasModulate/PointLight2D.enabled = false
	
# set road and play animation
func set_road(roomz):
	var roads = roomz
	...
	var road = preload("res://Forest/Misc/road.tscn").instantiate()
	add_child(road)
	...
	print(m.position)
	road.position = isom(start_x, start_y) 
	
	var start_x1 = roads.positions.position.x + 8
	var start_y1 = roads.positions.position.y + 15
	var roadss = preload("res://Forest/Misc/road.tscn").instantiate()
	add_child(roadss)
	roadss.position = isom(start_x1, start_y1)
	...
	m.t.play("edet")
	m.s.play()
	while m.position > endpos-Vector2(384, 192):
		m.position -= Vector2(6.4, 3.2)
		await get_tree().create_timer(0.05).timeout
	m.s.stop()
	print(m.position)
	...
	while $AndreiKulek/Camera2D.zoom < Vector2(2, 2):
		...
		#$CanvasModulate/PointLight2D.texture_scale -= 0.025
		await get_tree().create_timer(0.05).timeout
...
func _on_dialogic_signal(str):
	if str == "Start music":
		...
		#$CanvasModulate/PointLight2D.enabled = true
		$MainMusic.play()
		is_marsh = false
...
func process_rooms():
	if current_room == rooms[rooms.size()-1]:
		...
		if pos.y < current_room.positions.position.y+4 and forest:
			change_level_forest()
...
func _on_alex_button_pressed():
	...
	if player == $AndreiKulek:
		...
		$AndreiKulek/Camera2D.enabled = false
		$AlexandrUtkin/Camera2D.enabled = true
		#var zz = $AndreiKulek.position
		$AndreiKulek.position = Vector2(-1000, -1000)
		#await get_tree().create_timer(0.01).timeout
		#$AlexandrUtkin.position = zz
