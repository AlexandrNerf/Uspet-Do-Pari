...
func animation_predict(input_direction):
	...
	var rt = Input.get_vector("attack_left", "attack_right", "attack_up", "attack_down")
	if rt.x != 0 or rt.y != 0:
		rot = (rt).angle()
...
func _physics_process(delta):
	...
	if !changing and Dialogic.current_timeline == null:
		get_input(delta)
		var dirs = Input.get_vector("attack_left", "attack_right", "attack_up", "attack_down")
		move()
		if (Input.is_action_just_pressed("attack") or dirs.length_squared() > 0.25) and !changing and !onice:
...
func _on_bodybox_area_entered(area):
	if area.is_in_group("Marsh"):
		dead = true
	if !hitting and area.is_in_group("Slimes"):
		hitting = true
		...
		hitting = false
	elif !hitting and is_instance_valid(area) and area.is_in_group("Cone"):
		hitting = true
		_animated_sprite.modulate = Color(1, 0.5, 0.5, 1)
		damage_in_seconds = 7
		await get_tree().create_timer(0.5).timeout
